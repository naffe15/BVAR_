...
for hh = 0 : hor
    ...
    if dummy == 2 % activating Bayesian Direct methods.
        ...
        if hh == 0
            ...
            % constant
            G        = [prior.Phi.mean(end, :)'; zeros(ny * (lags-1), 1)];
            ...
            Fhh       = F^hh;
            Ghh       = (iIminusF * (eye(nylags)-Fhh)) * G;
            priorMean = [Fhh(1:ny,:)'; Ghh(1 : ny,1)'];
            
            prior.df  = prior.Sigma.df; % usually number of regressors minus the 2
            % prior.XXi = inv(prior.Phi.cov(hh * ny + 1 : hh * ny + 1 : ));
            prior.XXi = inv( prior.Phi.cov * 1/prior.tau(hh) );% \ one(nylags-1); % nendolags * nendo
            prior_int = matrictint(prior.Sigma.scale, prior.df, prior.XXi); % ny * ny
            
            posterior.df    = olsreg_(hh).N - ny*lags - nx + prior.Sigma.df;
            posterior.S     = olsreg_(hh).error * olsreg_(hh).error' + prior.Sigma.scale ;
            %         posterior.S     = olsreg_.Serror + prior.Sigma.scale ;
